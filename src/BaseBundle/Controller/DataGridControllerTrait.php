...
//dump($getData);exit;
        ...
        if(!$this->container->get('user.permissions')->accessLevel(['SUPER_ADMIN', 'admin']))
        {
          //  $getData['criteries']['createdBy'] = $this->getUser()->getId();
                ...
                foreach ($settings['action_links'] as $type => $link) {
                    ...
                    switch ($type) {
                        ...
                        case 'delete':
                            ...
                            break;
                        case 'view':
                            $action_links[$type.'_link'] = ['html' => '<a href="'.$link.'" class="btn btn-sm btn-clean btn-icon btn-icon-md" title="'.$translator->trans('View').'"> <i class="la flaticon-eye"></i></a>'];
                            break;
                        case 'renewal':
                            $action_links[$type.'_link'] = ['html' => '<a href="'.$link.'" class="btn btn-sm btn-clean btn-icon btn-icon-md" title="'.$translator->trans('Renewal').'"> <i class="la la-plus-circle"></i></a>'];
                            break;
                        case 'download':
...
//dump($result);exit;
    ...
    private function exportCSV($tableData, $settings, $entityClass, $fieldNames, $slug) {
       ...
       $phpExcelObject->getProperties()->setCreator("liuggio")
        ...
        ->setCellValue('A1', '№');
        $hn = 0;
        foreach($temp[0] as $head) {
            $hn = $hn+1;
            $phpExcelObject->getActiveSheet()
                ->setCellValue($cellsName[$hn].'1', $head);
        }
           // output rows
        if (!empty($settings['export']['csv']['row']) && is_callable($settings['export']['csv']['row'])) {
            ...
            // col names are default
            $rows = 2;
            foreach ($tableData['aaData'] as $key => $row) {
                $phpExcelObject->getActiveSheet()->setCellValue('A'.$rows, ($key+1));       
                $cells = 1;
                foreach ($fieldNames as $field=>$name) {
                    if ($field!='actions') {
                        $phpExcelObject->getActiveSheet()
                            ->setCellValue($cellsName[$cells].$rows, strip_tags($row[$field])); 
                        $cells = $cells+1;      
                    }
                }
              $rows++;
