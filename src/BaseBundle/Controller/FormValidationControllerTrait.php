...
trait FormValidationControllerTrait {
    ...
    public function xhrValidateEntity($e) {
        ...
        //echo "ERRCNT: ".count($errorsValidator);exit;
        $errors = [];
        foreach ($errorsValidator as $field => $error) {
            //$m = get_class_methods($error);print_r($m);foreach($m as $mm) { if (strstr($mm,'__') || in_array($mm, ['getParameters'])) continue; echo "D: $mm -> ".$error->{$mm}()."\n"; } exit;
            //echo "D: $field -> $error\n";
            $errors[$error->getPropertyPath()] = $error->getMessage();
        }
        if (sizeof($errors)>0) {
            //return formErrors for complatibility
            return ['formErrors' => $errors];
        }
        return false;
    }
    ...
    public function formErrors(FormInterface $form) {
        ...
        //print_r($form->getData());exit;
            ...
            $fname = $form->getName();
            foreach ($errorsValidator as $field => $error) {
                //print_r(get_class_methods($error));exit;
                //Array ( [0] => __construct [1] => __toString [2] => getMessageTemplate [3] => getParameters [4] => getPlural [5] => getMessage [6] => getRoot [7] => getPropertyPath [8] => getInvalidValue [9] => getConstraint [10] => getCause [11] => getCode )
                //var_dump($error->getRoot()->getPropertyPath());exit;
                $field = $error->getPropertyPath();
                $field1 = $error->getPropertyPath();
                //echo 'D:'.$field1.'->'.$fname.'->'.$error->getMessage()."\n";//exit;
                //if (strstr($field, 'children['))
                ...
                //TODO: regex for nested forms example user_form[user.plainPassword.first]
                if(strpos($field, 'user_form[user.') !== false) {

                  $field = str_replace('.', '][', $field);
                }
                if(strpos($field, 'document_form[rows.') !== false) {
                  $field = str_replace('.', '][', $field);
                }
                //echo "\n<br>\n".$field;exit;
