...
function selectBnomType(el) {
  ...
  initTree($('#kform_id_BaseBundle_basenoms .k-portlet__body'));
        ...
        success:function(result) {
          if(result['result']!=0){
            ...
            if(result['extraFields']!=null&&pass==null){
              var ael=null;
              if( 'doNotShowLastEl' in treeSettings && treeSettings['doNotShowLastEl'] != true) {
                ael=$('#BaseBundle_basenoms_name');
                addExtra(result['extraFields'],ael);
              }
            }

            $('#level_'+nomLevel).attr('name','BaseBundle_basenoms[parent_id]');
          }
        }
      })
  }
...
function createTree(data){
     sp ='<div id="element_'+data['level']+'" class="baseNomsParents">'
         ...
         +'      <select class="form-control" name="BaseBundle_basenoms[parent_id][]" id="level_'
                 +data['level']+'" level='+data['level']+'  im-insert="true" aria-describedby="type-error"></select>'
         +'    </div>'
         +'  </div>'
         +'</div>';
     $('#kform_id_BaseBundle_basenoms .k-portlet__body div.nomtree').append(sp);
 }
   ...
   if (data['type'] in BaseNomLoad)
    ...
    $('.baseNomName').remove();
     sp ='<div id='+$.now()+' class="baseNomsParents baseNomName">'
         ...
         +' name="BaseBundle_basenoms[name]" value="'+loadValue+'" />'
         +'</div></div>'
     ...
     if ($('#kform_id_BaseBundle_basenoms .k-portlet__body div.nameflds').length<1) {
...
console.log('kv',key,'v',value);
     ...
     sp = '<div  id ="ex_'+key+'" class="baseNomNameExtra">'
         ...
         +'    <div></div></div>';
     if ($('#kform_id_BaseBundle_basenoms .k-portlet__body div.extraflds').length<1) {
       if ($('div.nameflds').length >0)
       $('div.nameflds').append('<div class="extraflds"></div>');
         else
       $('#kform_id_BaseBundle_basenoms .k-portlet__body').prepend('<div class="extraflds"></div>');
     }
...
(function() {
 ...
 //load nom types
  $(document).on('change', '#BaseBundle_basenoms_type', function(){
    //selectBnomType($(this));
  });
...
//select paretn
 ...
 //submit handler
$(document).on('submit', '#kform_id_BaseBundle_basenoms', function(e){
      e.preventDefault();
    $('.error').remove();
      error = false;
      ...
      if(error){
        ...
        data = $('#kform_id_BaseBundle_basenoms').serialize();
        $.ajax({
