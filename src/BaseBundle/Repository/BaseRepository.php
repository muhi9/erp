...
// BaseBundle\Repository\BaseRepository
...
//echo $this->query->getQuery()->getDQL();exit;
//echo $this->query->getQuery()->getSQL();exit;
...
//dump($this->query->getQuery()->getResult());exit;
    ...
    public function getCount($data, $prepareQuery = false){
        ...
        */
        ...
        $this->eAlias = isset($data['alias'])&&!empty($data['alias'])?$data['alias']:'q';
        $this->qb = $this->createQueryBuilder($this->eAlias);
        $this->query = $this->qb;//->select($this->qb->expr()->count($this->eAlias));
        $this->query->select('count(q.id) as allRows');
        $this->query->addSelect($this->eAlias);
        ...
        //$r = $this->query->getQuery()->getSingleScalarResult();
...
*/
    ...
    public function findParentTree(\BaseBundle\Entity\BaseNoms $nom, array &$tree=[], $depth=0) {
      //$nomType = $nom->getType();//$this->_em->getRepository('BaseBundle:NomType')->find($nom->getType());
      $tree[$nom->getType()->getId()] = [ 'id' => $nom->getId(), 'value' => $nom->getName(), 'type' => $nom->getType()->getId(), 'name' => $nom->getType()->getName()];
      if ($nom->getParentId()) {
        $parentNom = $nom->getParent();//$this->_em->getRepository('BaseBundle:BaseNoms')->find($nom->getParentId());
